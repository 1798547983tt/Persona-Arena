...
- **剧情罗盘**：选世界书条目、导入 TXT 小说（逐段提要再汇总）或手写原著梗概，让剧情顾问根据原著、舞台记录与导演备注推演——什么必然发生、什么已脱离原著、什么已不可能、什么可能、蝴蝶效应、接下来的节拍与给正文的引导；引导自动注入正文提示词（可设深度/角色、可手改），可每 N 层自动更新，演员出手时也能看到走向。
- **史官（主 AI 动态更新）**：正文每出一楼，指定的主 AI 核对每位演员的心情、目标、羁绊、能力得失与人设变化（性格/外貌/经历/口吻/目标），写进本聊天的"本剧中的变化"，附 NPC 名册；有日志、可撤销、可固化进全局人设卡。
- **破限提示词**：内置你自己预设里的「破限」区条目（尼伯龙根之印 / 夜钟 / 严格誊录协议），开关自选，开启后作为所有插件调用的最顶端 system 提示；也可导入自己的预设 JSON 自动提取「破限」区。
...
6. **剧情**页：选原著（世界书 / TXT / 手写）、写导演备注，点「推演走向」，引导会自动注入正文提示词；设置里开启**史官**后，每出一楼演员数据会自动更新。
...
| 回合、面板、沙龙、私语、罗盘、史官日志、NPC 名册 | 当前聊天的 `chat_metadata['persona-arena']`（随聊天文件；分支会复制） |
...
- 史官只在酒馆渲染出新楼时触发（`CHARACTER_MESSAGE_RENDERED`）；重生成/滑动同一楼会再记一次，可撤销。
