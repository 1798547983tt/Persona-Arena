...
| 剧情罗盘 | 原著幕目 |
|---|---|
| ![plot](docs/screenshots/plot.png) | ![acts](docs/screenshots/plot-acts.png) |

| 史官更新后的演员 | 沙龙 |
|---|---|
| ![actor](docs/screenshots/actor-dynamic.png) | ![salon](docs/screenshots/salon.png) |

| 宣纸（浅色） | 手机 |
|---|---|
| ![paper](docs/screenshots/paper-stage.png) | ![mobile](docs/screenshots/mobile-stage.png) |
...
- **剧情罗盘（同人向）**：原著来源——按作品名生成（凭模型知识，可联网补资料）、从世界书选条目、导入 TXT 小说、手写梗概、导入别人分享的原著文件。原著不是一团梗概，而是**幕目**：按阶段分成若干**幕**，每幕若干条**剧情点**（TXT 走"逐段提取剧情点 → 合并成节 → 模型按阶段分幕 → 每幕整理剧情点 → 写概览"，可并发、可中止、可续跑；手写/世界书原著可一键"自动分幕"；幕与剧情点都能手改、增删、导出）。推演前先由模型**根据最近正文判断故事进行到哪一幕**（也可手动指定），把"本幕剧情点 + 上一幕要点 + 下一幕"交给顾问。剧情顾问读原著、最近 N 层正文、世界书与导演备注，推演：当前局势与原著进度、什么必然发生、什么已脱离原著、什么已不可能、什么可能、蝴蝶效应、**原著接下来本该发生的事（如期/提前/推迟/已不可能/已变形）**、**OOC 提醒**、接下来的节拍与给正文的引导。引导自动注入正文提示词（可设深度/角色、可手改），可每 N 层自动更新，演员出手时也能看到走向。
...
要求 SillyTavern ≥ 1.13。安装后右下角会出现一个写着「戏」的悬浮球。当前版本 1.1.0。
...
6. **剧情**页：写同人最省事的路径是「按作品名生成」原著（也可从世界书选条目、导入 TXT、手写），看一眼幕目，写导演备注，点「推演走向」——顾问会先判断故事进行到哪一幕，再推演，引导自动注入正文提示词；设置里开启**史官**后，每出一楼演员数据会自动更新。
...
| 原著 | 索引、概览与幕目存 `extension_settings['persona-arena'].canons`（跟着酒馆账号跨设备）；幕目超过约 8 万字时只存浏览器 IndexedDB；TXT 原文与总结进度只存 IndexedDB |
...
src/
  ...
  social.js           沙龙与私语
  search.js           联网搜索与人设生成
  canon.js            原著：TXT → 幕与剧情点、分幕、定位当前幕、导入导出
  plot.js             剧情罗盘：世界书条目、走向推演、注入
...
- 长篇 TXT 按约 6000 字一段提取剧情点，每 6 段合成一节，再由模型分幕；百万字约 200 次调用，五百万字约 1000 次。请用便宜的大上下文模型；中途可中止，同一文件再次导入会跳过已完成的段。
- 幕目太大（约 8 万字以上）时只存在当前浏览器；换浏览器打开会看到"尚未分幕"，用「导出原著」文件带过去即可。
